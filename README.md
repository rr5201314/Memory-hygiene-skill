...
项目细节、工具 pitfall、环境配置这类只在特定场景才需要的信息，每轮都占 token。比如你有一个 Warframe 项目的紫卡数据编辑规范，但它在你写论文、查资料、调代码时全都注入了——白花 token，还干扰模型注意力。
...
│  ┌───────────────────────────────────────────────────┐ │
│  │ name: cshzd-overview                              │ │
│  │ desc: 仓鼠互助洞项目总览。了解项目背景、技术栈时加载。  │ │
│  ├───────────────────────────────────────────────────┤ │
│  │ name: cshzd-riven-data                            │ │
│  │ desc: 紫卡数据编辑规范。编辑 import-data 时加载。     │ │
│  ├───────────────────────────────────────────────────┤ │
│  │ name: obsidian-setup-pitfalls                     │ │
│  │ desc: Obsidian 插件安装坑点。配置 Obsidian 时加载。  │ │
│  └───────────────────────────────────────────────────┘ │
│                                                         │
│  ↑ agent 扫描索引，根据当前任务语义判断是否需要加载 ↓     │
└─────────────────────────────────────────────────────┘

          ↓ skill_view(name="cshzd-riven-data") ↓
...
│  ├── 三组文件格式不同，不能互相覆盖                      │
│  ├── NDJSON 用 separators=(',',':')                   │
│  ├── 同步流程：import-data → cloudfunctions → static    │
│  └── ...                                                │
└─────────────────────────────────────────────────────┘
```
